...
function Posts() {
  ...
  const createPost = (e, title, body) => {
    e.preventDefault();
    posts.push({ userId: currentUser[0].userId, id: posts[posts.length - 1].id + 1, title: title, body: body })
    setPosts([...posts]);
  }
    ...
    <div className="mt-4 container">
      ...
      {!isLoaded ? <p>Loading...</p> :
        <div className="posts p-4">
          {posts.map(post => {
            return <Post post={post} key={post.id} deletePost={deletePost} updatePost={updatePost} />
          })}
        </div>}

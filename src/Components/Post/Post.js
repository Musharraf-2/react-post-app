...
function Post (props) {
  ...
  const deleteComment = (commentId) => {
    const deletedComment = props.comments.filter(comment => comment.commentId !== commentId)
    props.setComments(deletedComment)
  }

  const createComment = (comment, postId) => {
    props.comments.push({ commentId: props.comments[props.comments.length - 1].commentId + 1, postId, username: CURRENT_USER.username, email: CURRENT_USER.email, body: comment })
    props.setComments([...props.comments])
  }
